...
| [HuikeAgent 主干道 + 插件架构](docs/agent/huikeagent-trunk-and-plugins.md) | 把整个视觉软件重建成 Agent 可调用的形状：8 个扩展维度、7 个封闭动词、586 条守卫，相机已接真机。**Agent 那侧最大的欠账已填**：10 条场景 + 自动评分，Claude / 本地 qwen3:8b 都 10/10 过 Phase 1（裸模型不微调即达标）。§4 有 35 条可引用的硬经验——4.27–4.30 讲评测抓到的两个缺陷、投影必须能重算、判分对账为何不用版本号；4.31 讲一个改动如何把潜伏竞态从"撞不上"推到"必现" | `HuikeAgent` | 活跃 | 2026-08-25 |
