...
| [HuikeAgent 主干道 + 插件架构](docs/agent/huikeagent-trunk-and-plugins.md) | 把整个视觉软件重建成 Agent 可调用的形状：8 个扩展维度、7 个封闭动词、629 条守卫，相机已接真机。**Agent 那侧最大的欠账已填**：12 条场景 + 自动评分，Claude / 本地 qwen3:8b 都全过 Phase 1（裸模型不微调即达标），模型还看得见"人正在编辑的那张图"和"人站在哪一页"。§4 有 46 条可引用的硬经验——4.27–4.30 讲评测抓到的两个缺陷、投影必须能重算、判分对账为何不用版本号；4.31 讲一个改动如何把潜伏竞态推到"必现"；**4.35–4.41 讲"机制做出来了但界面上一个字都没有"、手势判据要模仿手、第三次把推断当选型依据、"值没变也算改了"如何把布局推成死循环、"模型能不能读到 X"要先确认 X 存在、以及接上了不等于用得上（8B 不预取就不去读）**；**4.42 讲"尺子自带答案"——评测抄了一份产品的预取清单，于是"16/16、达到验收线"量的是抄的那份** | `HuikeAgent` | 活跃 | 2026-08-27 |
