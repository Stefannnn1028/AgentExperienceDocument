...
| [HuikeAgent 主干道 + 插件架构](docs/agent/huikeagent-trunk-and-plugins.md) | 把整个视觉软件重建成 Agent 可调用的形状：8 个扩展维度、7 个封闭动词、629 条守卫，相机已接真机。**Agent 那侧最大的欠账已填**：12 条场景 + 自动评分，Claude / 本地 qwen3:8b 都全过 Phase 1（裸模型不微调即达标），模型还看得见"人正在编辑的那张图"和"人站在哪一页"。§4 有 43 条可引用的硬经验——4.27–4.30 讲评测抓到的两个缺陷、投影必须能重算、判分对账为何不用版本号；4.31 讲一个改动如何把潜伏竞态推到"必现"；**4.35–4.39 讲"机制做出来了但界面上一个字都没有"、手势判据要模仿手、第三次把推断当选型依据、"值没变也算改了"如何把布局推成死循环、以及落点判断要连可视区一起判** | `HuikeAgent` | 活跃 | 2026-08-27 |
