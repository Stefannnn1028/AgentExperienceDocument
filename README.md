...
| [HuikeAgent 主干道 + 插件架构](docs/agent/huikeagent-trunk-and-plugins.md) | 把整个视觉软件重建成 Agent 可调用的形状：8 个扩展维度、7 个封闭动词、629 条守卫，相机已接真机。**Agent 那侧最大的欠账已填**：12 条场景 + 自动评分，Claude / 本地 qwen3:8b 都全过 Phase 1（裸模型不微调即达标），模型还看得见"人正在编辑的那张图"和"人站在哪一页"。§4 有 38 条可引用的硬经验——4.27–4.30 讲评测抓到的两个缺陷、投影必须能重算、判分对账为何不用版本号；4.31 讲一个改动如何把潜伏竞态推到"必现"；**4.32–4.34 讲一个假绿的三重同时成立、变异检查自己也会漏、以及"信息到了"不等于"结论出来了"** | `HuikeAgent` | 活跃 | 2026-08-26 |
