...
| 文档 | 一句话 | 关联项目 | 状态 | 更新 |
|---|---|---|---|---|
| [图像标注交互规范](docs/engineering/image-annotation-interaction-spec.md) | 13 种标注类型 + 统一交互模型，从 `Draw.cs` 提炼成跨渲染库的契约：命中优先级、四个屏幕空间常量、旋转矩形九点模型、卡尺四参数模型。§6 有 13 条可引用的硬经验——6.3 讲 Chromium 的假 mouseleave 如何让拖拽看起来是坏的，6.4 讲逐帧累加为何会悄悄偏出几个像素 | `HuikeAgent`、`个人工作台` | 活跃 | 2026-08-26 |
