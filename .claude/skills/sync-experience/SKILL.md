...
2. **找到对应文档**。读 REPO 下所有 `docs/**/*.md` 的 front matter，匹配 `project_paths` 命中当前项目路径的那一份。
   - 命中一份 → 增量更新它。
   - 命中多份 → 问用户更新哪份。
   - **没命中** → 这是新项目，走「新增文档」流程（见下）。

3. **扫描变化**。以该文档 front matter 的 `last_updated` 为起点：
   ```
...
4. **调研变化**。派 Explore 代理读变化的文件（并行，按模块分组），要求它们只报告**有经验价值**的东西：
...
5. **增量更新文档**，严格遵守 `REPO\CONVENTIONS.md` §3：
...
6. **同步 README 索引**：更新该文档所在主题表格里的「一句话」「状态」「更新」列。新增文档则加一行；开了新主题则加一节并补「主题分类」表。
...
8. **汇报**：说清楚扫了哪些文件、改了哪几节、提交的 commit hash。无变化也要明说。
