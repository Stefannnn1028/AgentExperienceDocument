...
topic: testing
project_paths:
  - 'E:\Project\嘉立讯标准软件\自研算子\OperatorAutoTest'
  - 'E:\CarveYu\carvedyu\tools\CY3DOpTest'
status: 活跃
...
> 板端（包裹检测/灰度仪）那套见 `E:\Document\算子测试体系总结.md` 第四章，本文不重复。
> 配套文档：`E:\Document\DiagAgent现场诊断助手项目经验交接.md`（同批生成）。
...
| 工具位置 | `E:\Project\嘉立讯标准软件\自研算子\OperatorAutoTest\` | `E:\CarveYu\carvedyu\tools\CY3DOpTest\` |
...
| 内容 | 路径 | 说明 |
|---|---|---|
| 2D 测试图库 | `E:\Project\嘉立讯标准软件\测试图\自研算子测试图片\{1,2,3}` | 20/17/24 张产线 NG 图，子文件夹=产品。没有它只能跑合成图冒烟 |
...
| **外层仓库整目录** | `E:\Project\嘉立讯标准软件\自研算子\` | ⚠ **外层仓库没有任何 git remote**，推不出去。迁移只能整目录拷（含 `.git` 和未提交的工作区改动），否则 DiagAgent 那批已验证未提交的改动直接丢 |
| 3D 点云测试数据（可选） | `--cloud` 指定的真实点云文件 | 不给就不注册 realdata 那条用例 |

### 2.2 工具链

- **VS 2022 Preview 的 MSBuild**：`C:\Program Files\Microsoft Visual Studio\2022\Preview\MSBuild\Current\Bin\MSBuild.exe`。
...
```
E:\Project\嘉立讯标准软件\自研算子\        ← 外层仓库（无 remote，纯本地）
...
- 工程 `E:\CarveYu\carvedyu\tools\CY3DOpTest\`：`Program.cs`(CLI) / `TestHarness.cs`(断言·Golden·Report) / `Synthetic.cs`(合成数据) / `Suites.cs`(全部用例)。
...
17. **装一个真的第三方件 = 上真机**（同上，与 `Agent诊断助手经验.md` §6.2 同源）。把示例插件从"测试夹具目录"真装进产品的 `plugins/` 目录，一次跑出 4 个夹具路径测不出的缺陷：宿主的"漏翻就红"和"号段只能落在 01xx–08xx"两条自检**把本工程的内部约定套到了第三方的码上**（装个只写英文的插件，宿主自检就红——责任指向完全错的方向）；一处 `plugin!` 把"插件已装在别处"掩成 NullReferenceException；还有一条自检的**预期本身写错了**（已装在别处时，重扫夹具报"同 ID 两目录"恰恰是正确行为）。**判据：凡是"我们自己的编号规矩/翻译责任"那类断言，都要先问一句"这条数据是我们的吗"。**
...
| carvedyu | `E:\CarveYu\carvedyu` `main` = `ce3ec89` | 3D 相关文件自 08-06 未动 |

⚠ **carvedyu 有两个分叉副本**：`E:\CarveYu\carvedyu` 是活跃主线（3D 体系、`docs\adr`、`New3D` 都在这），`E:\Project\HuikeSmart\carvedyu` 停在 2026-05（只有 `docs\ai-chat` 评测集）。**别在错的副本上改 3D。**
...
| 测试图库 | `E:\Project\嘉立讯标准软件\测试图\自研算子测试图片\{1,2,3}`（不在仓库） |
| VS MSBuild | `C:\Program Files\Microsoft Visual Studio\2022\Preview\MSBuild\Current\Bin\MSBuild.exe`（主工程只能用它） |
